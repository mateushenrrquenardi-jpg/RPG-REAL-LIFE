...
function getHero() {
  ...
  return {
    nivel: row[0], exp_atual: row[1], exp_necessaria: row[2],
    forca: row[3], magia: row[4], carisma: row[5], inteligencia: row[6],
    data_limite: row[7]
  };
}
...
function addQuest(params) {
  ...
  // tipo pode ser: side, principal, diaria
...
function completeQuest(params) {
  ...
  if (!quest) return { error: 'Quest não encontrada' };

  // Marca como concluída com data/hora atual
  ...
  let [nivel, expAtual, expNec, forca, magia, carisma, inteligencia] = heroSheet.getRange(2, 1, 1, 7).getValues()[0];
  expAtual += expGanho;
  ...
  heroSheet.getRange(2, 1, 1, 7).setValues([[nivel, expAtual, expNec, forca, magia, carisma, inteligencia]]);
  histSheet.appendRow([new Date().toLocaleString('pt-BR'), quest.nome, expGanho, quest.atributo, pontosAtrib, nivel]);

  return { success: true, levelUp, nivel, expAtual, expNec, forca, magia, carisma, inteligencia, expGanho, pontosAtrib, atributo: quest.atributo };
}
...
function deleteQuest(params) {
  ...
  return { error: 'Quest não encontrada' };
}